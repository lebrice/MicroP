...
\begin{tikzpicture}[node distance=5cm,on grid,auto] 
 \node[punkt, color=blue] (discovery) {Discovery};
 \node[punkt, right=of discovery] (nucleo) {Nucleo};
 \node[punkt, right=of nucleo] (phone) {Phone};
...
\begin{tikzpicture}[node distance=5cm,on grid,auto] 
 \node[punkt] (discovery) {Discovery};
 \node[punkt, color=blue, right=of discovery] (nucleo) {Nucleo};
 \node[punkt, right=of nucleo] (phone) {Phone};
 \node[punkt, right=of phone] (server) {Flask Server};
   \path[<->] 
   		(discovery) edge[below] node {UART} 	(nucleo)
...
height=2cm, text width=3cm, node distance=3cm]
   ...
   \path[->]
   		...
   		(send_mic) edge[bend left] node {$sent < 1000$} (wait_2)
   		(send_mic) edge[above right, bend right] node {$sent==1000$} (wait)
...
\begin{tikzpicture}
    [%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        float/.style={rectangle,draw=black, ultra thick, minimum size=1cm, minimum width=2cm},
        byte/.style={rectangle,draw=black, thin, minimum size=0.25cm, minimum width=0.5cm},
    ]%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\foreach \x/\y in {0/$p_1$, 2/$r_1$,4/$p_2$,6/$r_2$,8/-}
        \node[float] at (\x,0){\y};

\draw[decorate,decoration={brace,mirror},thick] (-1,-.7) -- node[below]{Sample 1} (2.8,-.7);
\draw[decorate,decoration={brace,mirror},thick] (3.2,-.7) -- node[below]{Sample 2} (6.8,-.7);
\draw[decorate,decoration={brace,mirror},thick] (7.2,-.7) -- node[below]{Unused} (9,-.7);
...
\begin{tikzpicture}
    [%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        short/.style={rectangle,draw=black, ultra thick, minimum size=1cm, minimum width=1cm},
        byte/.style={rectangle,draw=black, thin, minimum size=0.25cm, minimum width=0.5cm},
    ]%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\foreach \x in {0, 1, ..., 9}
        \node[short] (m\x) at (\x,0){$m_{\x}$};
...
\begin{tikzpicture}[node distance=5cm,on grid,auto] 
 \node[punkt] (discovery) {Discovery};
 \node[punkt, right=of discovery] (nucleo) {Nucleo};
 \node[punkt, color=blue, right=of nucleo] (phone) {Phone};
 \node[punkt, right=of phone] (server) {Flask Server};
   \path[<->] 
   		(discovery) edge[below] node {UART} 	(nucleo)
        (nucleo) 	edge[below] node {BLE} 		(phone)
...
\begin{tikzpicture}[node distance=5cm,on grid,auto] 
 ...
 \node[punkt, right=of discovery] (nucleo) {Nucleo};
 \node[punkt, right=of nucleo] (phone) {Phone};
 \node[punkt, color=blue, right=of phone] (server) {Flask Server};
   \path[<->] 
   		(discovery) edge[below] node {UART} 	(nucleo)
        (nucleo) 	edge[below] node {BLE} 		(phone)
        (phone) 	edge[below] node {HTTP} 	(server)
