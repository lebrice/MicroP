...
uint16_t acc_char_handle, mic_char_handle, return_digit_char_handle;
...
// TODO: Find a unique UUID for the MIC characteristic.
...
// TODO: Find a unique UUID for the MIC characteristic.
#define COPY_RETURN_DIGIT_CHAR_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x05,0x36,0x6e,0x80, 0xcf,0x3a, 0x11,0xe1, 0x9a,0xb4, 0x00,0x02,0xa5,0xd5,0xc5,0x1b)
...
tBleStatus Add_Custom_Acc_Service(void)
{
  ...
  if (ret != BLE_STATUS_SUCCESS) goto fail;
	...
	COPY_RETURN_DIGIT_CHAR_UUID(uuid);  
  ret =  aci_gatt_add_char(
		custom_service_handle,
		UUID_TYPE_128, 
		uuid,
